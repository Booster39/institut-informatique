...
## Module 5 (30 cours) - Interface utilisateur moderne et frameworks frontend (Python) (version enseignant)

### Résumé
Ce module enseignant reprend l'intégralité du module étudiant avec les solutions, explications et points de correction.
...
1. UI = interface utilisateur (éléments visuels, boutons, layout). UX = expérience utilisateur (flux, ergonomie, satisfaction).
2. Streamlit est le meilleur pour prototypes rapides ; Gradio ajoute des composants ML et Dash pour dashboards interactifs.
3. Responsive design signifie que l’interface s’adapte à la taille du terminal (mobile, tablette, desktop), généralement via layout adaptatif et CSS flexible.
...
## Annexes enseignant
### Clés de correction et réponses du module 5
- Cours 1 : UI vs UX; Framework prototypage = Streamlit; responsive = design adaptatif selon ratio d'écran.
- Cours 3 : endpoint API pour Streamlit doit utiliser `requests.get('<API>/tasks')`; vérifier le JSON, afficher via `st.dataframe()`.
- Cours 7 : commandes Dash `dcc.Graph` et callback `@app.callback`.
- Cours 10 : tests automatisés via `pytest` + `pytest-httpx` pour endpoint; simulation de l'utilisateur.
- Cours 14 : instructions de déploiement Docker compose unifiant API, UI, et base.
- Cours 20 : QCM la performance est mesurée avec `streamlit run --server.maxUploadSize`, cache `@st.cache_data`.

### Erreurs fréquentes
- Ne pas rafraîchir les données après mutation (POST/PUT). Solution : `st.experimental_rerun` ou re-chargement bool.
- UI lente car `requests.get` dans la boucle. Solution : `st.cache_data` et retrieval batched.
- CORS non activé pour app Dash + API Flask. Solution : `Flask-Cors` sur API.

### Critères de notation (soutenance)
- Démonstration complète de 3 cas d’utilisation (créer, lister, filtrer tasks)
- Accessibilité (contraste, éléments keyboard-friendly)
- Responsive et test mobile
- Couverture tests API + UI > 75%
- Mise en production avec Docker

---

**Feedback enseignant**
- Encouragez la révision du code, la pair-programming et le README détaillé.
- Assurez-vous que la documentation mentionne tout usage des fichiers de config `.env`.
- Vérifiez la cohérence de l’architecture API/UI et les dépendances (`requests`, `streamlit`, `dash`, `plotly`, `pandas`).
